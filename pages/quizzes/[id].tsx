...
export default function Quiz({ quizData }: Props) {
    ...
    return (
        <div className={styles.container}>
                        ...
                        {images.map(({ url, text }, i) => {
                            return (
                                <figure key={i} className={styles.figure}>
                                    <img src={url}></img>
                                    <figcaption>Pic {i + 1} {shouldShowCaptions && <span>: {text}</span>}</figcaption>
                                </figure>)
                        })}
                    </div>
            ...
            <div>
                <button type="button" onClick={showCaptions}>Hint 1: Show image captions</button>
                <button type="button" onClick={showHint}>Hint 2: Even more suggestions</button>
            </div>
        </div>
    )
}
